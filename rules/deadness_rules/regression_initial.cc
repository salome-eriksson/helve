...
std::unique_ptr<Knowledge> regression_initial(SetID stateset_id,
    ...
    if (s_reg->get_stateset_id() != stateset_id) {
    ...
    const StateSetNegation *s_not =
            proof_checker.get_set<StateSetNegation>(subset_knowledge->get_right_id());
    if(s_not->get_child_id() != stateset_id) {
        throw std::runtime_error("The right side of subset knowledge #"
                                 + std::to_string(premise_ids[2])
...
DeadnessRule _ri_plugin("ri", regression_initial);
}
